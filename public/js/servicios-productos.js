...
doc.addEventListener("DOMContentLoaded",function(e){
      ...
      function botones(value, row, index) {
        ...
        function formatMoney(value, row, index){
            let valueConversion = new Intl.NumberFormat("es-CO").format(row.cantidad);  
    ...
    let pristine = new Pristine($myForm);     
     $myForm.addEventListener('submit',(e)=>{
        ...
        if(valid){
            ...
            axios.post('/productos-servicios',datos)
    ...
    document.addEventListener('click', (e)=>{
      ...
      if(e.target.matches('.editarData')){
        axios.get(`/edit/productos-servicios/${e.target.dataset.id}`)
         .then(function(response){
            let myModalEl = new bootstrap.Modal(document.getElementById('modalSignIn'), {
                keyboard: false
              });
              myModalEl.show();                      
              document.querySelector('.titulo-modal').textContent = 'Editar Productos x Servicios';
              ...
              document.getElementById('producto_id').value = response.data.data.producto_id;
              document.getElementById('cantidad').value = response.data.data.cantidad;
              document.getElementById('id').value = response.data.data.id;
              console.log( document.getElementById('servicio_id'));
      ...
      if(e.target.matches('.eliminarData')){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete/productos-servicios/${e.target.dataset.id}`).then(function(response){ 
                    notifications('Proceso exitoso!',response.data.message,'success'); 
                    setTimeout(()=>{
                     location.reload();
                    },2000)
      ...
      if(e.target.matches('.btn-modal')|| e.target.matches('.btn-cerrar')){
        $myForm.reset();
        pristine.reset();
        document.querySelector('.titulo-modal').textContent = 'Crear Producto x Servicio';
