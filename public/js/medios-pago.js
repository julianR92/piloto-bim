...
doc.addEventListener("DOMContentLoaded",function(e){
    ...
    document.addEventListener('click', (e)=>{
      ...
      if(e.target.matches('.editarData')){
        axios.get(`/edit/medios-pago/${e.target.dataset.id}`)
         .then(function(response){
            let myModalEl = new bootstrap.Modal(document.getElementById('modalSignIn'), {
                keyboard: false
              });
              myModalEl.show();                      
              document.querySelector('.titulo-modal').textContent = 'Editar Medios de Pago';
              document.querySelector('.btnModal').textContent = 'Editar';
              document.getElementById('medio_pago').value = response.data.data.medio_pago;
      ...
      if(e.target.matches('.eliminarData')){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete/medios-pago/${e.target.dataset.id}`).then(function(response){ 
                    notifications('Proceso exitoso!',response.data.message,'success'); 
                    setTimeout(()=>{
                     location.reload();
                    },2000)
      ...
      if(e.target.matches('.btn-modal')|| e.target.matches('.btn-cerrar')){
        $myForm.reset();
        pristine.reset();
        document.querySelector('.titulo-modal').textContent = 'Crear Medio de Pago';
