...
doc.addEventListener("DOMContentLoaded",function(e){
    ...
    document.addEventListener('click', (e)=>{
      ...
      if(e.target.matches('.editarData')){
        axios.get(`/edit/servicio/${e.target.dataset.id}`)
         .then(function(response){
            let myModalEl = new bootstrap.Modal(document.getElementById('modalSignIn'), {
              ...
              myModalEl.show();                      
              document.querySelector('.titulo-modal').textContent = 'Editar Tipo de Servicio';
              document.querySelector('.btnModal').textContent = 'Editar';
              document.getElementById('tipo_servicio_id').value = response.data.data.tipo_servicio_id;
              document.getElementById('servicio').value = response.data.data.servicio;
      ...
      if(e.target.matches('.eliminarData')){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete/servicio/${e.target.dataset.id}`).then(function(response){ 
                    notifications('Proceso exitoso!',response.data.message,'success'); 
                    setTimeout(()=>{
                     location.reload();
                    },2000)
      ...
      if(e.target.matches('.btn-modal')|| e.target.matches('.btn-cerrar')){
        $myForm.reset();
        pristine.reset();
        document.querySelector('.titulo-modal').textContent = 'Crear Servicio';
