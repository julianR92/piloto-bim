...
doc.addEventListener("DOMContentLoaded",function(e){
    ...
    let pristine = new Pristine($myForm);     
     $myForm.addEventListener('submit',(e)=>{
        ...
        if(valid){
            ...
            axios.post('/servicio-adicional',datos)
    ...
    document.addEventListener('click', (e)=>{
      ...
      if(e.target.matches('.editarData')){
        axios.get(`/edit/servicio-adicional/${e.target.dataset.id}`)
         .then(function(response){
            let myModalEl = new bootstrap.Modal(document.getElementById('modalSignIn'), {
              ...
              myModalEl.show();                      
              document.querySelector('.titulo-modal').textContent = 'Editar Serivicios Adicionales';
      ...
      if(e.target.matches('.eliminarData')){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete/servicio-adicional/${e.target.dataset.id}`).then(function(response){ 
                    notifications('Proceso exitoso!',response.data.message,'success'); 
                    setTimeout(()=>{
                     location.reload();
                    },2000)
      ...
      if(e.target.matches('.btn-modal')|| e.target.matches('.btn-cerrar')){
        $myForm.reset();
        pristine.reset();
        document.querySelector('.titulo-modal').textContent = 'Crear Servicio Adicional';
