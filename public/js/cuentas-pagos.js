...
doc.addEventListener("DOMContentLoaded",function(e){
      ...
      function estado(value, row, index) {        
         return [(row.estado)? 'ACTIVO✔️' : 'INACTIVO❌'].join('')     
    ...
    let pristine = new Pristine($myForm);     
     $myForm.addEventListener('submit',(e)=>{
        ...
        if(valid){
            ...
            axios.post('/cuentas-pagos',datos)
    ...
    document.addEventListener('click', (e)=>{
      ...
      if(e.target.matches('.editarData')){
        axios.get(`/edit/cuentas-pagos/${e.target.dataset.id}`)
         .then(function(response){
            let myModalEl = new bootstrap.Modal(document.getElementById('modalSignIn'), {
              ...
              myModalEl.show();                      
              document.querySelector('.titulo-modal').textContent = 'Editar cuentas de Pago';
              ...
              document.getElementById('entidad').value = response.data.data.entidad;
              document.getElementById('numero_cuenta').value = response.data.data.numero_cuenta;
      ...
      if(e.target.matches('.eliminarData')){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete/cuentas-pagos/${e.target.dataset.id}`).then(function(response){ 
                    notifications('Proceso exitoso!',response.data.message,'success'); 
                    setTimeout(()=>{
                     location.reload();
                    },2000)
      ...
      if(e.target.matches('.btn-modal')|| e.target.matches('.btn-cerrar')){
        $myForm.reset();
        pristine.reset();
        document.querySelector('.titulo-modal').textContent = 'Crear Cuenta de Pago';
