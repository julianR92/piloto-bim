...
doc.addEventListener("DOMContentLoaded",function(e){
    ...
    let pristine = new Pristine($myForm);     
     $myForm.addEventListener('submit',(e)=>{
        ...
        if(valid){
            ...
            axios.post('/tipo-servicio',datos)
    ...
    document.addEventListener('click', (e)=>{
      ...
      if(e.target.matches('.editarData')){
        axios.get(`/edit/tipo-servicio/${e.target.dataset.id}`)
         .then(function(response){
            let myModalEl = new bootstrap.Modal(document.getElementById('modalSignIn'), {
              ...
              document.getElementById('tipo_servicio').value = response.data.data.tipo_servicio;
      ...
      if(e.target.matches('.eliminarData')){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete/tipo-servicio/${e.target.dataset.id}`).then(function(response){ 
                    notifications('Proceso exitoso!',response.data.message,'success'); 
                    setTimeout(()=>{
                     location.reload();
                    },2000)
      ...
      if(e.target.matches('.btn-modal')|| e.target.matches('.btn-cerrar')){
        $myForm.reset();
        pristine.reset();
        document.querySelector('.titulo-modal').textContent = 'Crear Tipo Servicio';
