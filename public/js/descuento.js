...
doc.addEventListener("DOMContentLoaded",function(e){
      ...
      function botones(value, row, index) {        
        ...
        function discounts(value, row, index){           
            return [row.descuento+'%'].join('')
        }
    ...
    document.addEventListener('click', (e)=>{
      ...
      if(e.target.matches('.editarData')){
        axios.get(`/edit/descuentos/${e.target.dataset.id}`)
         .then(function(response){
            let myModalEl = new bootstrap.Modal(document.getElementById('modalSignIn'), {
              ...
              myModalEl.show();                      
              document.querySelector('.titulo-modal').textContent = 'Editar descuentos';
              document.querySelector('.btnModal').textContent = 'Editar';
              document.getElementById('plan').value = response.data.data.plan;
              document.getElementById('descuento').value = response.data.data.descuento;
      ...
      if(e.target.matches('.eliminarData')){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/delete/descuentos/${e.target.dataset.id}`).then(function(response){ 
                    notifications('Proceso exitoso!',response.data.message,'success'); 
                    setTimeout(()=>{
                     location.reload();
                    },2000)
      ...
      if(e.target.matches('.btn-modal')|| e.target.matches('.btn-cerrar')){
        $myForm.reset();
        pristine.reset();
        document.querySelector('.titulo-modal').textContent = 'Crear descuentos';
